 ...
 * @since 1.2
 */
public class ObjectSelectAction extends BaseSQLAction {
    ...
    protected FluentSelect<?> query;
    protected QueryMetadata queryMetadata;

    /**
     * @since 4.0
     */
    public ObjectSelectAction(FluentSelect<?> query, DataNode dataNode) {
        super(dataNode);
        this.query = query;
    ...
    private <T> ResultIterator<T> forFetchLimit(ResultIterator<T> iterator, SelectTranslator translator) {
        ...
        int fetchLimit = query.getLimit();
        int offset = translator.isSuppressingDistinct() ? query.getOffset() : getInMemoryOffset(query
                .getOffset());
