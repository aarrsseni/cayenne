...
public class DefaultObjectSelectTranslator extends QueryAssembler implements SelectTranslator {
     ...
     */
    DefaultSelectTranslator.AddJoinListener joinListener;


    public DefaultObjectSelectTranslator(Query query, DbAdapter adapter, EntityResolver entityResolver) {
    ...
    protected void doTranslate() {
        ...
        // build qualifier
        QualifierObjectSelectTranslator qualifierTranslator = adapter.getQualifierObjectSelectTranslator(this);
        StringBuilder whereQualifierBuffer = qualifierTranslator.appendPart(new StringBuilder());

        // build having qualifier
//        Expression havingQualifier = ((FluentSelect)query).getHavingQualifier();
...
//        }
    ...
    private FluentSelect<?> getSelectQuery() {
        return (FluentSelect<?>) getQuery();
    }
    ...
    protected List<ColumnDescriptor> buildResultColumns() {
        ...
        FluentSelect<?> query = getSelectQuery();
...
//        } else
     ...
     */
//    <T> List<ColumnDescriptor> appendOverriddenColumns(List<ColumnDescriptor> columns, FluentSelect<T> query) {
//        groupByColumns = new HashMap<>();
//
//        QualifierTranslator qualifierTranslator = adapter.getQualifierTranslator(this);
//        DefaultSelectTranslator.AccumulatingBindingListener bindingListener = new DefaultSelectTranslator.AccumulatingBindingListener();
...
//    }
    ...
    <T> List<ColumnDescriptor> appendDbEntityColumns(List<ColumnDescriptor> columns, FluentSelect<T> query) {

        Set<DefaultSelectTranslator.ColumnTracker> attributes = new HashSet<>();
    ...
    <T> List<ColumnDescriptor> appendQueryColumns(final List<ColumnDescriptor> columns, FluentSelect<T> query, ClassDescriptor descriptor, final String tableAlias) {

        final Set<DefaultSelectTranslator.ColumnTracker> attributes = new HashSet<>();
...
//        }
        ...
        if (query.getPrefetches() != null) {
            ...
            for(PrefetchTreeNode prefetch : query.getPrefetches().getChildren()) {
                prefetch.setEntityName(oe.getName());
            }

            for (PrefetchTreeNode prefetch : query.getPrefetches().adjacentJointNodes()) {
    ...
    <T> List<ColumnDescriptor> appendIdColumns(final List<ColumnDescriptor> columns, ObjEntity objEntity) {
        ...
        Set<DefaultSelectTranslator.ColumnTracker> skipSet = new HashSet<>();
    ...
    private void appendColumn(List<ColumnDescriptor> columns, ObjAttribute objAttribute, DbAttribute attribute,
                              Set<DefaultSelectTranslator.ColumnTracker> skipSet, String label) {
        appendColumn(columns, objAttribute, attribute, skipSet, label, null);
    }

    private void appendColumn(List<ColumnDescriptor> columns, ObjAttribute objAttribute, DbAttribute attribute,
                              Set<DefaultSelectTranslator.ColumnTracker> skipSet, String label, String alias) {

        if(alias == null) {
            alias = getCurrentAlias();
        }

        if (skipSet.add(new DefaultSelectTranslator.ColumnTracker(alias, attribute))) {
...
//        Collection<Property<?>> columns = ((FluentSelect<?>)query).getColumns();
//        if(columns == null) {
//            return null;
//        }
...
//        }
    ...
    static final class ColumnTracker {
        ...
        public boolean equals(Object object) {
            if (!(object instanceof DefaultSelectTranslator.ColumnTracker)) {
                return false;
            }

//            DefaultSelectTranslator.ColumnTracker other = (DefaultSelectTranslator.ColumnTracker) object;
