...
All filtering tags _<includeTable>_, _<excludeTable>_, _<includeColumn>_, _<excludeColumn>_, _<includeProcedure>_ and _<excludeProcedure>_ have 2 ways to pass filtering RegExp.
...
All filtering tags can be placed inside schema and catalog tags, but also inside _<dbimport>_ tag. It means that filtering rules will be applied for all schemas and catalogs.
...
Here is config sample for _Ant_ task:
...
NOTE: In Ant task configuration all filter tags located inside root tag _<cdbimport>_ as there is no _<dbimport>_ tag.
